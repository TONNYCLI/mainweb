        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            ...
            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                ...
                <button class="btn bg-indigo-500 hover:bg-indigo-600 text-white">
                    ...
                    <a href="{{ route('admin/new/photos' ) }}">
                        <span class="hidden xs:block ml-2">Add photo</span>
                                    ...
                                    <td class="p-2 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="w-10 h-10 shrink-0 mr-2 sm:mr-3">
                                                <img class="rounded-full" src="{{ asset($photo->file_path) }}" width="40" height="40" alt="Alex Shatov" />
                                            </div>
                                            <div class="font-medium text-slate-800">Alex Shatov</div>
                                        </div>
                                    </td>
                                    <td class="p-2 whitespace-nowrap">
                                        <div class="text-left">{{ $photo->details }}</div>
                                    </td>
                                    <td class="p-2 whitespace-nowrap">
                                        <div class="text-left font-medium text-green-500">{{ $photo->title }}</div>
                                    </td>
                                    <td class="p-2 whitespace-nowrap">
                                        <div class="text-lg text-center">{{ $photo->created_at }}</div>
                                    </td>
                                    <td class="p-2 whitespace-nowrap">
                                        <a href="{{ route('edit/photo', $photo->id ) }}"><div class="btn bg-indigo-500 hover:bg-indigo-600 text-white whitespace-nowrap">edit</div></a>
                                    </td>
                                    <td class="p-2 whitespace-nowrap">
                                        <form action="{{ route('destroy/photo', $photo->id ) }}" method="post" >
                                            @csrf
                                            @method('delete')
